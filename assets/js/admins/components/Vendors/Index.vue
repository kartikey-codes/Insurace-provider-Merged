	...
	<div>
			...
			>
				<template v-slot:[`cell(name)`]="data">
					...
					<span v-else class="text-uppercase font-weight-bold text-danger">
						<font-awesome-icon icon="exclamation-triangle" fixed-width />Missing
					</span>
				</template>
		...
		<b-container v-else-if="!error">
			<empty-result class="my-5">
				No vendors were found matching your criteria
				<template #content>
	...
	methods: {
		async refresh(page = null) {
			...
			try {
				...
				const response = await this.$store.dispatch("vendors/index", requestParams);
