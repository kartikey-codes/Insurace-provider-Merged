<template>
	...
	>
			...
			<b-row>
				<b-col cols="12">
					<b-collapse v-model="filtering" class="py-2">
						<b-form @submit.prevent="doSearch">
							<b-card>
								<IndexFilters v-model="filters" :disabled="loading" hide-assigned-to hide-status />
							</b-card>
						</b-form>
					</b-collapse>
				</b-col>
		...
		<b-container fluid v-else class="my-4">
			<empty-result>
				No unassigned appeals found
				<template #content> No unassigned appeals have been created or match your search. </template>
			</empty-result>
		</b-container>
	</paginated-results>
</template>
...
const filters = reactive({ status: "Open", assigned_to: false, ...defaultFilters });
