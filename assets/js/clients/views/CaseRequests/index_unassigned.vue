	...
	>
			...
			<b-row>
				<b-col cols="12">
					<b-collapse v-model="filtering" class="py-2">
						<b-form @submit.prevent="doSearch">
							<b-card>
								<IndexFilters v-model="filters" :disabled="loading" hide-status hide-assigned-to />
							</b-card>
						</b-form>
					</b-collapse>
				</b-col>
		...
		<b-container fluid v-else class="my-4">
			<empty-result>
				...
				<template #content> No unassigned requests have been created or match your search. </template>
			</empty-result>
		</b-container>
	</paginated-results>
</template>
