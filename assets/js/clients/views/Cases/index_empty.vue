<template>
	...
	>
			...
			<b-row>
				<b-col cols="12">
					<b-collapse v-model="filtering" class="py-2">
						<b-form @submit.prevent="doSearch">
							<b-card>
								<IndexFilters v-model="filters" :disabled="loading" hide-empty />
							</b-card>
						</b-form>
					</b-collapse>
				</b-col>
		...
		<b-container fluid v-else class="my-4">
			<empty-result>
				No empty cases found
				<template #content> No empty cases have been created or match your search. </template>
			</empty-result>
		</b-container>
	</paginated-results>
</template>
...
const filters = reactive({ ...defaultFilters, empty: true });
