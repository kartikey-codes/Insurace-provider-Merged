				...
				<b-card-footer>
					<b-row>
						<b-col cols="12" md="6" xl="4" class="mb-2 mb-md-0">
							<b-button block variant="light" @click.prevent="cancel">Cancel</b-button>
						</b-col>
	...
	methods: {
		...
		async refresh() {
			try {
				...
				const response = await this.$store.dispatch("denialReasons/get", {
					id: this.id,
				});
				...
				this.$store.dispatch("apiError", {
					error: e,
					message: "Error getting denial reason details",
				});
			} finally {
				this.loading = false;
			}
		...
		async save(e) {
			try {
				...
				const response = await this.$store.dispatch("denialReasons/save", this.entity);
				...
				this.$store.dispatch("apiError", {
					error: e,
					title: "Save Failed",
					message: "Error saving denial reason details. Please check for errors.",
					variant: "warning",
				});
			} finally {
				this.saving = false;

				this.$store.dispatch("denialReasons/getAll");
			}
		},
	},
};
